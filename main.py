...
run_button = st.sidebar.button("Run/Reset")
...
def plot_dataset(data):
    ...
    plot_placeholder.plotly_chart(global_fig, config={'displayModeBar': False})
...
def plot_dbscan(X, labels, core_point_idx=None, final=False):
        ...
        if len(cluster_points) > 0:
            ...
            if label != 0 and label != -1:
                global_fig.add_trace(go.Scatter(
                    ...
                    y=cluster_points["Feature 2"],
                    mode='markers',
                    marker=dict(size=eps * 1000, color=color, opacity=0.03, line=dict(width=0)),
                    marker_color=marker_color,
                    name=None,
    ...
    if final:
        plot_placeholder.plotly_chart(global_fig, key="final", config={'displayModeBar': False})
    else:
        plot_placeholder.plotly_chart(global_fig, config={'displayModeBar': False})
